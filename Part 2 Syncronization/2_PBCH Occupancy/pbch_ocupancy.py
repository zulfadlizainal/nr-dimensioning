...
# PBCH Occupancy/Sec = ((PBCH Size)*L*SSB Burst Count in 1 Sec)/Total RE per Sec

pbch_size = (240*1*2) + (48*1*2)
sec = 1000
            ...
            if i < len(p):
                ...
                if l <= 8:
                    ...
                    df_fr1[f'L = {l}, p = {p[i]} ms'] = (
                        ((pbch_size)*l*(np.floor(sec/p[i])))/df_fr1['Normal CP RE/Sec'])*100
            ...
            if i < len(p):
                ...
                if l <= 64:
                    ...
                    df_fr2[f'L = {l}, p = {p[i]} ms'] = (
                        ((pbch_size)*l*(np.floor(sec/p[i])))/df_fr2['Normal CP RE/Sec'])*100
...
df_fr1_scs15_5ms.plot(kind='line', style='.--', ax=axes[0,0], ylim=[0,25], xlim=[0,100])
df_fr1_scs15_10ms.plot(kind='line', style='.--', ax=axes[0,1], ylim=[0,25], xlim=[0,100])
df_fr1_scs15_20ms.plot(kind='line', style='.--', ax=axes[0,2], ylim=[0,25], xlim=[0,100])
...
fig1.suptitle('PBCH Occupancy (%) for FR = 1, SCS = 15 kHz\nL = SSB Count in SSB Burst Set, p = SSB Periodicity')
...
df_fr1_scs30_5ms.plot(kind='line', style='.--', ax=axes[0,0], ylim=[0,25], xlim=[0,100])
df_fr1_scs30_10ms.plot(kind='line', style='.--', ax=axes[0,1], ylim=[0,25], xlim=[0,100])
df_fr1_scs30_20ms.plot(kind='line', style='.--', ax=axes[0,2], ylim=[0,25], xlim=[0,100])
...
fig2.suptitle('PBCH Occupancy (%) for FR = 1, SCS = 30 kHz\nL = SSB Count in SSB Burst Set, p = SSB Periodicity')
...
df_fr2_scs120_5ms.plot(kind='line', style='.--', ax=axes[0,0], ylim=[0,25], xlim=[0,400])
df_fr2_scs120_10ms.plot(kind='line', style='.--', ax=axes[0,1], ylim=[0,25], xlim=[0,400])
df_fr2_scs120_20ms.plot(kind='line', style='.--', ax=axes[0,2], ylim=[0,25], xlim=[0,400])
...
fig3.suptitle('PBCH Occupancy (%) for FR = 2, SCS = 120 kHz\nL = SSB Count in SSB Burst Set, p = SSB Periodicity')
