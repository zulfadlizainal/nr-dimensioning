...
df_se[f'{eg1}'] = ((cell_tput_one.iloc[0]['PDSCH-RE/Frame'] * radio_frame_sec) * (df_se['Estimated SE (bps/Hz)'] * ((cell_tput_one.iloc[0]
                   ['SCS (kHz)']*1000)/(normal_cp_symbols * cell_tput_one.iloc[0]['Slots/Frame']*100))) / 1000000) * (1 + dl_mu_mimo_gain) * (1 - dl_bler)
df_se[f'{eg2}'] = ((cell_tput_two.iloc[0]['PDSCH-RE/Frame'] * radio_frame_sec) * (df_se['Estimated SE (bps/Hz)'] * ((cell_tput_two.iloc[0]
                   ['SCS (kHz)']*1000)/(normal_cp_symbols * cell_tput_two.iloc[0]['Slots/Frame']*100))) / 1000000) * (1 + dl_mu_mimo_gain) * (1 - dl_bler)
df_se[f'{eg3}'] = ((cell_tput_three.iloc[0]['PDSCH-RE/Frame'] * radio_frame_sec) * (df_se['Estimated SE (bps/Hz)'] * ((cell_tput_three.iloc[0]
                   ['SCS (kHz)']*1000)/(normal_cp_symbols * cell_tput_three.iloc[0]['Slots/Frame']*100))) / 1000000) * (1 + dl_mu_mimo_gain) * (1 - dl_bler)
df_se[f'{eg4}'] = ((cell_tput_four.iloc[0]['PDSCH-RE/Frame'] * radio_frame_sec) * (df_se['Estimated SE (bps/Hz)'] * ((cell_tput_four.iloc[0]
                   ['SCS (kHz)']*1000)/(normal_cp_symbols * cell_tput_four.iloc[0]['Slots/Frame']*100))) / 1000000) * (1 + dl_mu_mimo_gain) * (1 - dl_bler)


# Plot

df_se[[f'{eg1}', f'{eg2}', f'{eg3}', f'{eg4}']].plot(
    style=['v--', 's--', '8--', 'h--'], markersize=3, markerfacecolor='None', color=['firebrick', 'limegreen', 'steelblue', 'indigo'])
plt.title(
...
plt.xlim(-10, 20)
plt.grid()
plt.legend(title='Type (FR)_SCS (kHz)_BW (MHz)', loc='upper center',
           bbox_to_anchor=(1.28, 1), fancybox=True)
