...
class pagingvolume:
    ...
    def paging_ran(mo_rrc_hour, mt_rrc_hour, rrc_inactive_hour, ran_noti_area):
        paging_demand_ran = (mo_rrc_hour + mt_rrc_hour) / \
            3600 * rrc_inactive_hour * ran_noti_area
        return paging_demand_ran


class pagingoccasion:

    # Number of paging occassion per second
    def paging_occassion(N, Ns):
        ...
        for i in range(len(N)):
            for j in range(len(Ns)):
                ...
                df_po.loc[index, ['po']] = N[i] * Ns[j] * 100             # 100 radio frame per second
                index = index + 1
        return df_po
...
class pagingue:
    ...
    # Number of UE ID per paging message
