...
def paging_re(paging_count):

    pc = paging_count

    # Calculate RE consumption for paging (Calculate number of bits needed and convert to RE count based on code rate table)
    for i in range(len(ssb_beams)):
        pc[f'{ssb_beams[i]}'] = pc[f'{ssb_beams[i]}'] * ((ue_identity_bits * pc['ue_per_paging_msg']) + paging_header_bits) * (
            code_rate_constant / code_rate_value / modulation_bitspersym)
    ...
    plt.title(
        f'[5G NR]\nSA Paging PDSCH RE / Radio Frame\n\nAssumptions (within TAC/RNA Area):\nMT RRC User in Busy Hour = {mt_rrc_hour}\nCells in TAC Area = {tac_size}\nCells in RAN Notification Area = {ran_noti_area}\nRRC Inactive Time per One RRC Session = {rrc_inactive}\n\nCode Rate = {code_rate_value}\nModulation Bits/Symbol = {modulation_bitspersym}\nUE Identity Bits in Paging Message = {ue_identity_bits}\nPaging Message Header Bits = {paging_header_bits}\n')
    plt.ylabel('Paging PDSCH RE / Radio Frame')
    plt.xlabel('N_Ns Settings')
    plt.ylim(0)
    plt.xlim(0, 17)
    plt.grid()
    ...
    # plt.savefig('5G_SA_PagingREperFrame.png', dpi=300, bbox_inches='tight')
    plt.show()
